...
server.put("/:id/actions/:action_id", validateProjectId, (req, res) => {
    const { id, action_id } = req.params;

    const updatedAction = req.body;

    if (updatedAction.description && updatedAction.description.length > 128) {
        res.status(400).json({
...
server.delete("/:id/actions/:action_id", validateProjectId, (req, res) => {
    ...
    actions
        ...
        .catch(err =>
            res
                .status(500)
                .json({ errorMessage: `Error deleting action ${action_id}` })
        );
});
